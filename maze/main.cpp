...
bool moveCharacterAndCheckIfExitFound(std::array<std::array<char, mazeColumns>, mazeRows> &prMaze, bool &rKeySymbol)
{
    ...
    if (scanForChar(prMaze, characterSymbol, charRow, charColumn))
    {
        ...
        {
            gameMessage("Key found!");
            rKeySymbol = true;
        }

        if (charMovedOnto == exitSymbol && rKeySymbol == true)
        {
...
void playMazeGame()
{
    ...
    while (!moveCharacterAndCheckIfExitFound(maze, rKeySymbol));
}
